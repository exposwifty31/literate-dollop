...
import { removeConflict, subscribeConflicts } from "@/lib/conflict-store";
import {
...
export function SyncProvider({ children }: { children: ReactNode }) {
  ...
  const failedCount = useMemo(
    () =>
      allItems.filter((i) => i.status === "failed" || i.status === "dead" || i.status === "conflict")
        .length,
    [allItems],
  );
  ...
  const applyAll = useCallback((all: PendingSync[]) => {
    ...
    const f = all.filter(
      (i) => i.status === "failed" || i.status === "dead" || i.status === "conflict",
    ).length;
  ...
  // Reactive queue mirror — replaces the web Dexie `liveQuery`. Re-reads the
  // full queue (debounced) whenever a queue or conflict mutation is announced.
  useEffect(() => {
    let cancelled = false;
    ...
    reload();
    const unsubQueue = subscribeQueueChange(reload);
    const unsubConflicts = subscribeConflicts(reload);
