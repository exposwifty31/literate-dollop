import { getStoredLocale, isSupportedLocale, type Locale } from "@/lib/i18n";
import { safeStorageGetItem, safeStorageSetItem } from "@/lib/safe-storage";

/**
 * User settings persistence — ported from web `user-settings-storage.ts`.
 * Storage is backed by `safe-storage` (AsyncStorage cache) instead of
 * `localStorage`; the settings shape is unchanged.
 */
