...
import { interpolate } from "./i18n-core/interpolate";
import { isInternalKey } from "./i18n-core/internal-keys";
import { INITIAL_LOCALE, type Locale as SharedLocale } from "./i18n-core/types";
...
/**
 * RN replacement for the web `localStorage`-backed locale store.
 *
 * The web build read/wrote `localStorage` synchronously so `t` could be
 * built at module load. React Native has no synchronous persistent store,
 * so we keep an in-memory `currentLocale` (seeded with `INITIAL_LOCALE`)
 * that `getStoredLocale()` returns synchronously, and hydrate it from
 * AsyncStorage once at app startup via `hydrateStoredLocale()`.
 */
let currentLocale: Locale = INITIAL_LOCALE;

type LocaleChangeListener = (locale: Locale) => void;
const localeChangeListeners = new Set<LocaleChangeListener>();

/**
 * Subscribe to locale changes (replaces the web `vettrack:locale-changed`
 * CustomEvent). Returns an unsubscribe function.
 */
...
function emitLocaleChange(locale: Locale): void {
  for (const listener of localeChangeListeners) {
    try {
      listener(locale);
    } catch {
      // a listener throwing must never break locale propagation
    }
  }
}
...
/**
 * Synchronous read of the in-memory locale. Seeded with `INITIAL_LOCALE`
 * and updated by `hydrateStoredLocale()` / `setStoredLocale()`.
 */
...
/**
 * Hydrate the in-memory locale from AsyncStorage. Call once at app startup
 * (before the first render that depends on `t`). Returns the resolved locale.
 */
export async function hydrateStoredLocale(): Promise<Locale> {
  try {
    const stored = await AsyncStorage.getItem(LOCALE_STORAGE_KEY);
    const resolved = resolveClientLocale(stored);
    if (resolved !== currentLocale) {
      currentLocale = resolved;
      refreshTranslations(resolved);
      emitLocaleChange(resolved);
    }
    return resolved;
  } catch {
    return currentLocale;
...
export function setStoredLocale(locale: string): Locale {
  ...
  refreshTranslations(resolved);
  emitLocaleChange(resolved);
  // Persist asynchronously; a storage failure must not break the UI update.
  void AsyncStorage.setItem(LOCALE_STORAGE_KEY, resolved).catch(() => {});
  return resolved;
}
...
/**
 * RN equivalent of the web `applyLocaleDocumentAttributes`. Aligns the
 * native layout direction (RTL for Hebrew) via `I18nManager`. Toggling RTL
 * at runtime requires an app reload to fully take effect — callers decide
 * whether to prompt for restart.
 */
export function applyLocaleDirection(locale: string | null | undefined): void {
  const resolved = resolveClientLocale(locale);
  const shouldBeRtl = getDirection(resolved) === "rtl";
...
function buildTranslations(d: typeof heDict) {
  ...
  const translations = {
    ...
    equipmentList: {
      ...
      paginationCount: (shown: number, total: number) =>
        tr(d.equipmentList.paginationCount, { shown, total }),
      paginationPage: (page: number, pages: number) =>
        tr(d.equipmentList.paginationPage, { page, pages }),
    ...
    shiftHandoverPage: {
      ...d.shiftHandoverPage,
      pendingEmergenciesAlert: (count: number) =>
        tr(d.shiftHandoverPage.pendingEmergenciesAlert, { count }),
    ...
    inventoryPage: {
      ...
      finishRestockWithMissing: (missing: number) =>
        tr(d.inventoryPage.finishRestockWithMissing, { missing }),
    ...
    equipmentDetail: {
      ...
      scanCount: (count: number) =>
        count === 1 ? d.equipmentDetail.scanCountOnce : tr(d.equipmentDetail.scanCount, { count }),
      ...
      recoveryAttentionCalloutCheckedOutLong:
        d.equipmentDetail.recoveryAttentionCalloutCheckedOutLong,
      toast: {
    ...
    codeBlue: {
      ...
      reconciliation: {
        ...
        reconciledHeader: (count: number) =>
          tr(d.codeBlue.reconciliation.reconciledHeader, { count }),
      },
    },
    ...
    admin: {
      crashCart: {
        ...d.admin.crashCart,
        itemSubtitle: (key: string, qty: number) =>
          tr(d.admin.crashCart.itemSubtitle, { key, qty }),
    ...
    roomRadarPage: {
      ...
      filterHint: (shown: number, total: number) =>
        tr(d.roomRadarPage.filterHint, { shown, total }),
    },
    ...
    medsPage: {
      ...
      assignMedicationWithVolume: (volume: string) =>
        tr(d.medsPage.assignMedicationWithVolume, { volume }),
      assignMedicationWithTab: (tab: string) => tr(d.medsPage.assignMedicationWithTab, { tab }),
      deviationFromRecommended: (sign: string, pct: string) =>
        tr(d.medsPage.deviationFromRecommended, { sign, pct }),
      deviationBlocked: (sign: string, pct: string) =>
        tr(d.medsPage.deviationBlocked, { sign, pct }),
    ...
    erCommandCenter: {
      ...
      activeAssistance: {
        ...
        activePatient: (name: string) =>
          tr(d.erCommandCenter.activeAssistance.activePatient, { name }),
    ...
    crashCart: {
      ...
      checkItemAria: (label: string, checked: boolean) =>
        tr(d.crashCart.checkItemAria, {
          label,
          status: checked
            ? d.crashCart.checkItemStatusChecked
            : d.crashCart.checkItemStatusUnchecked,
        }),
      ...
      relativeHoursMinutes: (h: number, m: number) =>
        tr(d.crashCart.relativeHoursMinutes, { h, m }),
