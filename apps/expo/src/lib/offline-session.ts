...
/**
 * Offline session snapshot — ported verbatim from web `offline-session.ts`,
 * with the synchronous storage backed by `safe-storage` (AsyncStorage cache)
 * instead of `localStorage`. Hydrate via `hydrateSafeStorage()` at startup so
 * `restoreOfflineSession()` can read synchronously during auth bootstrap.
 */
...
function extractTokenExp(token: string): number {
  try {
    ...
    // `atob` is available in both Hermes (RN) and Node 18+.
